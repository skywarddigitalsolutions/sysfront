...
export default function UsuariosDashboard() {
    ...
    const { data: users = [], refetch } = useQuery({
        ...
        queryFn: () => fetchUsers(filterActive),
    })
    ...
    const deactivateMutation = useMutation({
        ...
        onError: (error: Error) => {
            toast({
                title: "Error",
                description: error.message || "No se pudo desactivar el usuario. Es posible que sea el último admin activo.",
                variant: "destructive",
            })
        },
    })
    ...
    const getRoleBadgeColor = (roleName: string) => {
        switch (roleName) {
            case "admin":
                return "bg-[#1E2C6D] text-white"
            case "cajero":
                return "bg-[#D9251C] text-white"
            case "cocina":
                return "bg-gradient-to-r from-[#1E2C6D] to-[#D9251C] text-white"
            default:
                return ""
        }
    }
    ...
    const handleCreateUser = () => {
        ...
        if (newUser.password.length < 6 || !passwordRegex.test(newUser.password)) {
            toast({
                ...
                <DialogContent className="bg-black border-white/20 text-white">
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button
                            ...
                            className="flex-1 bg-gradient-to-r from-[#1E2C6D] to-[#D9251C] text-white"
                        >
                            {resetPasswordMutation.isPending ? "Blanqueando..." : "Blanquear Contraseña"}
                        </Button>
                    </div>
                </DialogContent>
            </Dialog>
